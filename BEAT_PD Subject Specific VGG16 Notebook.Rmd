...
```{python}
dataset.run_test_preprocessing(specific_key_dataset=1004)
```
...
```{python}
# Start tensorboard
notebook.start('--logdir lightning_logs --host localhost --port 6965')
```

```{python}
# Display tensorboard
notebook.display(port=6964)
```
...
logger = TensorBoardLogger("lightning_logs", name="VGG16", version='subject_1004 on_off')
...
model = LightningVGG16(hparams,train_list=dataset.train_list, val_list=dataset.val_list,
                       label_class=label_class)
logger = TensorBoardLogger("lightning_logs", name="VGG16", version=f'subject_{subject_id} on_off')
trainer = pl.Trainer(early_stop_callback=True, max_epochs=10, gpus=1, logger=logger)
trainer.fit(model)
```
...
# Edit VGG16 For REAL Predictions
...
```{python}
# Run training/validation of model
model = LightningVGG16(hparams,train_list=REALdataset.train_list, val_list=REALdataset.val_list,
                       label_class=label_class)
logger = TensorBoardLogger("lightning_logs", name="VGG16", version=f'subject_{subject_id}_on_off')
trainer = pl.Trainer(early_stop_callback=True, max_epochs=10, gpus=1, logger=logger)
trainer.fit(model)
```
...
```{python}
# Predict and fill in submission template
for spec, id_, subject in REALdataset.test_data_list:
    output = model(torch.unsqueeze(TF.to_tensor(spec).cuda(), dim=0))
    prediction = output.argmax(dim=1, keepdim=True).item()
    df_index = tst_sc1.index[tst_sc1['measurement_id'] == id_]
    tst_sc1.prediction[df_index] = prediction
