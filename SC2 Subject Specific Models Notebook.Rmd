...
```{python}
tst_sc1 = pd.read_csv('Data/BEATPDchallenge/BEAT-PD_SC2_Dyskinesia_Submission_Template.csv')
```
...
```{python}
# Start tensorboard
notebook.start('--logdir lightning_logs --host localhost --port 7001')
```

```{python}
# Start tensorboard
notebook.display(port=7001)
```
...
logger = TensorBoardLogger("lightning_logs", name="VGG16", version=f'subject_{subject_id}_{label_class}')
