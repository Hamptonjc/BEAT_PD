...
```{python}
dataset.run_test_preprocessing()
```
...
```{python}
# Start tensorboard
notebook.start('--logdir lightning_logs --host localhost --port 6962')
```

```{python}
# Display tensorboard
notebook.display(port=6962)
```
...
logger = TensorBoardLogger("lightning_logs", name="VGG16", version='subject 1004 dyskines')
...
submission = pd.read_csv('Data/BEATPDchallenge/BEAT-PD_SC2_Dyskinesia_Submission_Template.csv')
submission.head()
```

```{python}
tst_id_ = pd.read_csv('Data/BEATPDchallenge/CIS-PD_Test_Data_IDs.csv')
tst_id_
```

```{python}
for measurement in submission.measurement_id:
