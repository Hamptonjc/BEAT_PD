...
print(f'it took {total/60} minutes to create dataset dictionary')
```
...
```{python}
model_name = "testing_psuedo_samples"
...
```{python}
dataset.run_preprocessing(specific_key_dataset=subject_id, create_psuedo_samples=True, k_neighbors=5)
```
...
```{python}
# Start tensorboard
notebook.start(f'--logdir lightning_logs --host localhost --port 2347')
```

```{python}
notebook.display(port=2347)
```
...
#tst_sc1.to_csv('Data/on_off_predictions.csv', index=False)
```

```{python}
# Check to see if csv saved correctly
#pd.read_csv('Data/on_off_predictions.csv').head()
```
