...
# BEAT-PD SC2 Subject Specific Models Notebook
...
# Identifying Sub Challenge 2 Test Subjects
...
```{python}
tst_sc2 = pd.read_csv('Data/BEATPDchallenge/BEAT-PD_SC2_Dyskinesia_Submission_Template.csv')
```

```{python}
tst_sc2['subject_id'] = np.nan
```

```{python}
for idx1, row1 in tst_sc2.iterrows():
    for idx2, row2 in pd.concat([cis_tst_id,real_tst_id]).reset_index(drop=True).iterrows():
        if row1.measurement_id == row2.measurement_id:
            tst_sc2.subject_id[idx1] = row2.subject_id
```

```{python}
 tst_sc2.subject_id.unique()
```
...
```{python}
model_name = "testing_score_function"
...
```{python}
# Start tensorboard
notebook.start(f'--logdir lightning_logs --host localhost --port 1336')
```

```{python}
notebook.display(port=1336)
```
...
for spec, id_, subject, ts in dataset.test_data_list:
    ...
    df_index = tst_sc2.index[tst_sc2['measurement_id'] == id_]
    tst_sc2.prediction[df_index] = prediction
```
...
```{python}
del tst_sc2['subject_id']
```

```{python}
tst_sc2.prediction = tst_sc2.prediction.astype(int)
```

```{python}
#Save submission df to CSV
#tst_sc2.to_csv('Data/dyskinesia_predictions.csv', index=False)
```

```{python}
# Check to see if csv saved correctly
#pd.read_csv('Data/dyskinesia_predictions.csv').head()
```
