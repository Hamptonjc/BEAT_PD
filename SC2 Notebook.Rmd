...
```{python}
# Start tensorboard
notebook.start(f'--logdir lightning_logs --host localhost --port 5421')
```

```{python}
notebook.display(port=5421)
```
...
tst_sc2.to_csv('Data/dyskinesia_predictions_final.csv', index=False)
```

```{python}
# Check to see if csv saved correctly
pd.read_csv('Data/dyskinesia_predictions_final.csv').head()
```
