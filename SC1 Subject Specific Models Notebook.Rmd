...
```{python}
# Run training/validation of model
model = LightningVGG16(dataset_name=dataset_name,hparams,train_list=dataset.train_list,
                       val_list=dataset.val_list,label_class=label_class)
logger = TensorBoardLogger("lightning_logs", name="VGG16", version=f'subject_{subject_id}_{label_class}')
trainer = pl.Trainer(early_stop_callback=True, max_epochs=10, gpus=1, logger=logger)
trainer.fit(model)
...
```{python}
# Run training/validation of model
model = LightningVGG16(dataset_name=dataset_name,hparams=hparams, val_list=dataset.val_list,
